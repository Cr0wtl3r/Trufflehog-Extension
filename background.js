...
const codeHeuristics = {
    ...
    "Automated Code Markers": "(?i)(generated by ai|chatgpt|github copilot|claude)",
...
const checkData = function (data, src, regexes, denyList, fromEncoded = false, parentUrl = undefined, parentOrigin = undefined) {
    ...
    for (const key in regexes) {
        const keyDenied = denyList.some(denied => {
            if (!denied) return false;
            const dl = denied.toLowerCase();
            const kl = key.toLowerCase();
            return kl === dl || kl.includes(dl) || regexes[key] === denied;
        });
        if (keyDenied) continue;
        ...
        try {
            ...
            if (match) {
                ...
                if (Array.isArray(matchText)) { matchText = matchText.toString(); }
                ...
                findings.push({ src, match: matchText, key, encoded: fromEncoded, parentUrl });
            }
        } catch (e) {
            console.error('[Trufflehog] Regex error for key:', key, e);
        }
    }
...
browserAPI.runtime.onMessage.addListener(function (request, sender, sendResponse) {
    browserAPI.storage.sync.get(['generics', 'specifics', 'aws', 'checkEnv', 'checkGit', 'checkEndpoints', 'highEntropy', 'codeHeuristics', 'appEnabled', 'checkSourceMaps'], function (config) {
        if (config.appEnabled === false) return;
                    ...
                    if (!skip) {
                        fetch(jsUrl, { credentials: 'include' })
                            ...
                            .then(data => {
                                ...
                                if (config.checkSourceMaps !== false) {
